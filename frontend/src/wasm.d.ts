declare module '/wasm/orbit_synth_wasm.js' {
  ...
  // review). The `paths` mapping in `frontend/tsconfig.json` points at
  // `src/wasm/orbit_synth_wasm.d.ts`, which CI populates from the
  // `wasm-orbit-pkg` artifact via `cp -r wasm-orbit-pkg/* frontend/src/wasm/`
  // (see `.github/workflows/pytest.yml::frontend-tests`). Locally the
  // same file is regenerated by `wasm-pack build --target web` from
